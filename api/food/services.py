...
class FoodService(ServiceBaseClass):
    ...
    def update_food_list(self, params=None, data=None, partial=False, **kwargs):
        ...
        if not 'many' in params or not 'ids' in params:
            ...
            return Exception({"message": "Missing parameters. Must contain \"many\" and \"ids\""}, status.HTTP_400_BAD_REQUEST)
        
        if not params["many"]:
            
            # many param is not True
            return Exception({"message": "\"many\" should be True"}, status.HTTP_400_BAD_REQUEST)
            
        ids_str = params['ids']
        if not re.match("\d+(?:,\d+)?", ids_str):
            
            # unauthorized characters
            return Exception({"message": "Unauthorized characters in \"ids\" parameter. Correct format is numbers separated by \",\" characters."}, status.HTTP_400_BAD_REQUEST)
        ...
        if not isinstance(ids, list):
            ...
            return Exception({"message": "Format error. \"ids\" should be a list."}, status.HTTP_400_BAD_REQUEST)
        
        if not isinstance(data, list) or len(ids) != len(data):
            
            # mismatch between ids param and request data
            return Exception({"message": "\"ids\" param length did not match request.data length"}, status.HTTP_400_BAD_REQUEST)
